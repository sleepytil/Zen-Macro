...
class macroActivity(customtkinter.CTk):
    def __init__(self):
        ...
        self.webhookURL = customtkinter.StringVar(
            self, self.config["Webhook"]["webhook_url"]
        )
        self.psURL = customtkinter.StringVar(
            self, self.config["Webhook"]["private_server"]
        )
        self.totalBiomesFound = int(self.config["Stats"]["total_biomes_discovered"])
        self.userID = None
        try:
            self.userID = customtkinter.StringVar(
                self, self.config["Webhook"]["discord_user_id"]
            )
        except Exception as e:
            ...
            self.write_config()
            pass
        self.multi_webhook = customtkinter.StringVar(
            self, self.config["Webhook"]["multi_webhook"]
        )
        self.aura_detection = customtkinter.IntVar(
            self, int(self.config["Macro"]["aura_detection"])
        )
        self.auraMin = None
        try:
            self.auraMin = customtkinter.StringVar(
                self, self.config["Macro"]["aura_min_rarity"]
            )
        except Exception as e:
            print(
                f"Error: No Minimum Aura Rarity found in config, skipping...\nDebug: {e}"
            )
        ...
        self.anti_afk = 0
        try:
            self.anti_afk = customtkinter.IntVar(
                self, int(self.config["Macro"]["anti_afk"])
            )
        except Exception as e:
            print(
                f"Error: No Anti-AFK setting found in config, skipping...\nDebug: {e}"
            )
            self.anti_afk = customtkinter.IntVar(self, 0)
            self.write_config()
            pass
        webhook_urls_string = customtkinter.StringVar(
            self, self.config["Webhook"]["multi_webhook_urls"]
        )
        ...
        updateInfo = customtkinter.CTkTextbox(
            tabview.tab("Home"), width=707, height=360, corner_radius=0
        )
        ...
        dev_label = customtkinter.CTkLabel(
            credits_frame,
            text="@sleepytil - Creator",
            font=customtkinter.CTkFont(family="Segoe UI", size=14, weight="bold"),
        )
        dev_label.grid(row=0, column=0, padx=(10, 0), sticky="nw")

        dev_link = customtkinter.CTkLabel(
            credits_frame,
            text="GitHub",
        ...
        dev_link.bind(
            "<Button-1>", lambda e: self.open_url("https://github.com/sleepytil")
        )
        ...
        webhook_label.grid(
            column=0, row=0, columnspan=2, padx=(10, 0), pady=(5, 0), sticky="w"
        )
        ...
        ps_label.grid(
            column=0, row=1, padx=(10, 0), pady=(20, 0), columnspan=2, sticky="w"
        )
        ...
        userid_label.grid(
            column=0, row=2, padx=(10, 0), pady=(20, 0), columnspan=2, sticky="w"
        )

        userid_field = customtkinter.CTkEntry(
            tabview.tab("Webhook"),
            font=customtkinter.CTkFont(family="Segoe UI", size=20),
            width=470,
        ...
        detection_toggle = customtkinter.CTkCheckBox(
            ...
            text="Aura Detection",
            font=customtkinter.CTkFont(family="Segoe UI", size=20),
            variable=self.aura_detection,
            command=self.auradetection_toggle_update,
        )
        detection_toggle.grid(
            row=1, column=0, columnspan=1, padx=(10, 0), pady=(10, 0), sticky="w"
        )

        auramin_label = customtkinter.CTkLabel(
            tabview.tab("Config"),
            text="Minimum Ping Rarity:",
            font=customtkinter.CTkFont(family="Segoe UI", size=20),
        )
        auramin_label.grid(
            column=0, row=2, padx=(10, 0), pady=(20, 0), columnspan=1, sticky="w"
        )

        anti_afk_toggle = customtkinter.CTkCheckBox(
            tabview.tab("Config"),
            text="Anti-AFK",
            font=customtkinter.CTkFont(family="Segoe UI", size=20),
            variable=self.anti_afk,
            command=self.anti_afk_toggle_update,
        )
        anti_afk_toggle.grid(
            row=0, column=0, columnspan=1, padx=(10, 0), pady=(10, 0), sticky="w"
        )

        auramin_field = customtkinter.CTkEntry(
            tabview.tab("Config"),
            font=customtkinter.CTkFont(family="Segoe UI", size=20),
            width=470,
            textvariable=self.auraMin,
        )
        auramin_field.grid(row=2, column=1, padx=(8, 0), pady=(23, 0), sticky="w")

        total_biomes_label = customtkinter.CTkLabel(
            tabview.tab("Stats"),
            text="Total Biomes Found:",
            font=customtkinter.CTkFont(family="Segoe UI", size=20),
        )
        total_biomes_label.grid(
            column=0, row=0, columnspan=2, padx=(10, 0), pady=(5, 0), sticky="w"
        )

        self.total_biomes_amount = customtkinter.CTkLabel(
            tabview.tab("Stats"),
            font=customtkinter.CTkFont(family="Segoe UI", size=20),
            text=f"{self.totalBiomesFound}",
        )
        self.total_biomes_amount.grid(
            column=1, row=0, columnspan=2, padx=(195, 0), pady=(5, 0), sticky="w"
        )
    ...
    def startMacro(self):
        ...
        if self.multi_webhook.get() != "1":
            if (
                "discord.com" not in self.webhookURL.get()
                or "https://" not in self.webhookURL.get()
            ):
            ...
            for line in self.file:
                if "[FLog::UpdateController] version response:" in line:
                    ...
                    try:
                        ...
                        print(
                            "Encountered error while parsing JSON to find Roblox update version."
                        )
                                ...
                                if event and event != last_event:
                                    if self.multi_webhook.get() != "1":
                                        ...
                                        if event == "NORMAL":
                                            ...
                                            self.totalBiomesFound = (
                                                int(self.totalBiomesFound) + 1
                                            )
                                            self.total_biomes_amount.configure(
                                                text=f"{self.totalBiomesFound}"
                                            )
                                            event_biome_colour = ""
                                            biomeEndingTime = ""
                                            if event in self.biome_colours:
                                                event_biome_colour = self.biome_colours[
                                                    event
                                                ]
                                            else:
                                                event_biome_colour = "FFFFFF"
                                            ...
                                            if (
                                                event in self.biome_times
                                                and event != "EGGLAND"
                                            ):
                                                biomeEndingTime = int(
                                                    time.time()
                                                ) + int(self.biome_times[event])
                                                biomeEndTimeFormatted = (
                                                    f"<t:{biomeEndingTime}:R>"
                                                )
                                                event_biome_colour = self.biome_colours[
                                                    event
                                                ]
                                            ...
                                            embed.set_thumbnail(
                                                url="https://sleepytil.github.io/biome_thumb/"
                                                + event.replace(" ", "%20")
                                                + ".png"
                                            )
                                            webhook.add_embed(embed)
    ...
    def auradetection_toggle_update(self):
        if self.aura_detection.get() == 1:
            self.popup(
                "You are about to be prompted to enable screen recording permissions for this application.\nThis is required for aura detection to work, "
    ...
    def anti_afk_toggle_update(self):
        if self.anti_afk.get() == 1:
            self.popup(
                "You are about to be prompted to enable accessibility permissions for this application.\nThis is required for the anti-AFK feature to work, "
    ...
    def stop(self):
        ...
        if self.started and not self.stopped:
            if self.multi_webhook.get() != "1":
                if (
                    "discord.com" in self.webhookURL.get()
                    and "https://" in self.webhookURL.get()
                ):
                    ending_webhook = discord_webhook.DiscordWebhook(
                        url=self.webhookURL.get()
                    )
                    ending_embed = discord_webhook.DiscordEmbed(
                        description=f"> ### Macro Stopped\n**Runtime**: `{str(datetime.timedelta(seconds=int((time.time() - self.macro_start_time))))}`\n**Join our Discord server**:\nhttps://discord.gg/xymDbw7jJV",
                        color="FF0000",
                    )
                    ...
                    ending_embed.set_thumbnail(
                        url="https://sleepytil.github.io/images/macrostop.png"
                    )
                    ending_embed.set_timestamp(
                        datetime.datetime.now(datetime.timezone.utc)
                    )
            ...
            if self.multi_webhook.get() != "1":
                if (
                    ...
                    try:
                        ...
                        embed_copy["image"] = {
                            "url": f"attachment://{os.path.basename(filename)}"
                        }
                        with open(filename, "rb") as image_file:
                            ...
                            data = {
                                "payload_json": json.dumps(
                                    {"content": content, "embeds": [embed_copy]}
                                )
                            }
                            requests.post(
                                self.webhookURL.get(),
                        ...
                        try:
                            print(
                                f"Failed to send inventory screenshot to {self.webhookURL.get()}: {e}"
                            )
                        except Exception:
                            pass
            else:
                ...
                if self.multi_webhook.get() != "1":
                    if (
                        ...
                        try:
                            ...
                            try:
                                print(
                                    f"Failed to send aura screenshot to {self.webhookURL.get()}: {e}"
                                )
                            except Exception:
                                pass
                else:
